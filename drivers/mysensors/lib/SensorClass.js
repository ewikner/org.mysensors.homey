...
let deviceClasses = require('./deviceclasses.json');
let mysensorsProtocol = require('./mysensorsProtocol');
...
class Sensor extends events.EventEmitter {
	...
	getDefaultCapability() {
		let defaultCapability = mysensorsProtocol.getCapabilities(this.sensorType);
	...
	setPayloadFromMessage(value) {
		let old_payload = this.payload;
		this.setPayload(value);
		let payload_parse = this.parsePayload();
	...
	parsePayload() {
		...
		let newValue = null;
		let thisCapability = this.getCapabilityType();
		if (thisCapability == null) {
			let defaultCapability = this.getDefaultCapability();
			if (defaultCapability == null) {
				return null;
			}
		...
		let deviceCapabilityObj = this.getDeviceClassesCapabilities();
		let capability = deviceCapabilityObj[thisCapability];
		if (capability) {
			switch (capability.type) {
				...
				case 'enum':
					if (('values' in capability)) {
						let enumValues = capability.values;
						var enumValue = null;
						for (var i = 0; i < enumValues.length; i++) {
							let eValue = enumValues[i];
							if (value == eValue.id) {
								enumValue = eValue.id;
								break;
							}
